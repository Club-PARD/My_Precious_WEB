  ...
  return (
    <div>
      {BorrowDataSet.map((data, index) => {
        ...
        const totalDebtMoney = data.debts.reduce(
          (acc, debt) => acc + Number(debt.lendMoney),
          0
        );

        const navigateToManage = (board_id) => {
          navigate(`/manage-request/${board_id}`);
        };
        ...
        return (
          <Container key={index} onClick={() => navigateToManage(board_id)}>
            <StyleCircleProgressbar>
              <CircleProgressbar
                totalMoney={data.borrowMoney}
                debtMoney={totalDebtMoney}
                index={index}
              />
            </StyleCircleProgressbar>
            <DisplayBorderBottom>
              <DetialsExplain>
                ...
                <UserDateTextDiv>
                  ...
                  <Row style={{ gap: "8px" }}>
                    <UserDataText>
                      {Number(data.borrowMoney).toLocaleString()}
                    </UserDataText>
                    <GrayText>원</GrayText>
                  </Row>
                  <Row style={{ gap: "3px" }}>
