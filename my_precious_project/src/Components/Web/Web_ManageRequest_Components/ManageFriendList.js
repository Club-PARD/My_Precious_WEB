...
function ManageFriendList({boardId, setGetdebtidShow}) {
    ...
    .then((response) => {
      console.log("데이터를 받아오는중: ", response);
        ...
        const numValues = debtDataArray.map((item, index) => {
            ...
            return { Debtid: num.id, Debtname: numName, DebtlendMoney: numLendmoney }
        });
    ...
    return (
        <ThemeProvider theme={theme}>
            <Container>
            {lendData.map((friend) => (
                <SummaryFriend key={friend.Debtid} 
