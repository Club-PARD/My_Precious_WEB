...
function LeftSide({ under100, setUnder100, updateLeftSide, boardId }) {
    ...
    const getData = async () => {
        try {
            const response = await axios.get(`https://httptest.dhdhh.shop/api/v23/boards/${boardId}`);
    ...
    return (
        <ThemeProvider theme={theme}>
            <Container>
                ...
                <ImageCharacter />
                <Image>
                    <ImageText>현재까지 {detailData.lendMoneyCount}명의 친구가 함께 도와주고 있어요.</ImageText>
                </Image>
                <StyleLineProgress>
                    <LineProgress
                ...
                <DisplayBoxDiv>
                    ...
                    <Line style={{ marginTop: '0.56rem' }}>
                        ...
                        <DisplayDataTotalDiv style={{ width: '23.1875rem' }}>
                            <DisplayDataTotalText>{detailData.bankAccount}</DisplayDataTotalText>
                        </DisplayDataTotalDiv>
                    </Line>
                    <SignDiv>
                        <SignText>서약</SignText>
                        <SignText>
                            나 {detailData.name}(은)는 {formatted_date}까지 돈을 갚을 것을 약속합니다. 감사합니다.
                        </SignText>
